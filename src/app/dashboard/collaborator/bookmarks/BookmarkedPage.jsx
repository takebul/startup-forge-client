      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-normal text-slate-500 bg-slate-800/60 border border-slate-700/50 px-2.5 py-0.5 rounded-full">
              Collaborator
            </span>
          </h2>
        ...
        <span className="text-xs font-mono text-slate-400 bg-[#0D1528] border border-slate-800 px-3.5 py-1.5 rounded-xl w-fit shadow-sm">
          Total Saved:{" "}
          ...
          {bookmarks.map((item, idx) => {
            ...
            return (
              <div
                ...
                className="rounded-2xl p-5 bg-[#0D1528] border border-slate-800 hover:border-slate-700/80 transition-all duration-200 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 shadow-sm"
              >
                {/* Role Details */}
                <div className="min-w-0 flex-1 space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <h4 className="font-semibold text-sm text-slate-100 hover:text-indigo-300 transition-colors">
                    ...
                    item.requiredSkills.length > 0 && (
                      <div className="flex flex-wrap items-center gap-1.5 pt-1">
                        {item.requiredSkills.slice(0, 4).map((skill, sIdx) => (
                          <span
                            key={sIdx}
                            className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-white/5 border border-slate-700/60 text-slate-300"
                          >
                            {skill}
                          </span>
                        ))}
                ...
                <div className="flex items-center gap-2 shrink-0 pt-2 lg:pt-0 border-t lg:border-t-0 border-slate-800/80">
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 transition-all cursor-pointer disabled:opacity-50 flex items-center justify-center"
                  >
          ...
          <div className="space-y-5 font-sans">
            ...
            {selectedOpp.description && (
              <div>
                ...
                <p className="text-xs text-slate-300 leading-relaxed bg-[#060C1A] p-3 rounded-xl border border-slate-800">
                  {selectedOpp.description}
                </p>
              </div>
            )}

            <div className="grid grid-cols-2 gap-3 text-xs bg-[#060C1A] p-3.5 rounded-xl border border-slate-800">
              <div>
