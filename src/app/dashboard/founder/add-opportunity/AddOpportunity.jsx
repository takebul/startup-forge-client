    ...
    <div className="p-8 space-y-6 max-w-5xl mx-auto">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 mt-1">
          ...
          <span className="text-amber-500 font-semibold">@{startupName}</span>.
        </p>
      </div>

      {/* PLAN USAGE CARD WITH VIEW PLANS BUTTON */}
      <div className="rounded-2xl p-5 bg-[#0D1528] border border-slate-800/80 space-y-3 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold font-mono text-amber-500 bg-amber-500/10 border border-amber-500/20 px-2.5 py-0.5 rounded-lg">
                {planName}
              </span>
            </div>
            ...
            <span className="font-mono text-slate-300">
              <strong className={isLocked ? "text-red-400" : "text-amber-500"}>
                {createdCount}
              </strong>{" "}
              / {maxAllowed} Posted
            </span>
          ...
          <Button
            ...
            className="px-3.5 py-1.5 rounded-xl font-semibold text-xs bg-white/5 hover:bg-white/10 text-slate-300 border border-slate-800 transition-all cursor-pointer w-fit flex items-center gap-1.5"
          >
            <span>⚡</span>
        ...
        {/* Progress Line */}
        <div className="w-full bg-[#060C1A] border border-slate-800 rounded-full h-2 overflow-hidden">
          <div
            className={`h-full transition-all duration-500 rounded-full ${
              isLocked ? "bg-red-500" : "bg-amber-500"
            }`}
      ...
      {error && (
        <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs flex items-center justify-between">
          ...
          <button
            ...
            className="font-mono underline hover:text-red-300 cursor-pointer"
          >
            Dismiss
          </button>
        </div>
      ...
      {isLocked ? (
        <div className="rounded-2xl p-10 bg-[#0D1528] border border-amber-500/20 text-center space-y-5 shadow-lg">
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            <Button
              type="button"
              onClick={() => router.push("/pricing")}
              className="px-6 py-3 rounded-xl font-bold text-xs bg-amber-500 hover:bg-amber-600 text-slate-950 transition-all cursor-pointer shadow-md"
            >
              ⚡ Upgrade Your Plan
            </Button>
            <Button
              type="button"
              onClick={() => router.push("/pricing")}
              className="px-5 py-3 rounded-xl font-semibold text-xs bg-white/5 hover:bg-white/10 text-slate-300 border border-slate-800 transition-all cursor-pointer"
            >
              View All Plans
            </Button>
          </div>
        ...
        <Form
          ...
          className="rounded-2xl p-6 space-y-5 bg-[#0D1528] border border-slate-800"
        >
          {/* Role Title */}
          ...
          <div className="flex gap-3 pt-3 border-t border-slate-800">
            <Button
              type="submit"
              isDisabled={loading}
              className="px-6 py-2.5 rounded-xl font-semibold text-sm bg-amber-500 hover:bg-amber-600 text-slate-950 transition-all cursor-pointer disabled:opacity-50"
            >
