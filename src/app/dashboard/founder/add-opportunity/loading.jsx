"use client";

export default function AddOpportunityLoading() {
  return (
    <div className="p-4 sm:p-8 space-y-6 max-w-4xl font-sans">
      {/* Header Skeleton */}
      <div className="space-y-2">
        <div className="h-7 w-48 rounded-xl bg-slate-200/80 dark:bg-slate-800/80 animate-pulse" />
        <div className="h-4 w-72 rounded-lg bg-slate-200/60 dark:bg-slate-800/60 animate-pulse" />
      </div>

      {/* Form Container Skeleton */}
      <div className="rounded-3xl p-6 sm:p-8 bg-white border border-slate-200/90 shadow-sm dark:bg-slate-900/80 dark:border-slate-800/90 space-y-6">
        {/* Field 1: Role Title */}
        <div className="space-y-2">
          <div className="h-3 w-28 rounded bg-slate-200/80 dark:bg-slate-800/80 animate-pulse" />
          <div className="h-11 w-full rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 animate-pulse" />
        </div>

        {/* 2-Column Grid Fields */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            <div className="h-3 w-24 rounded bg-slate-200/80 dark:bg-slate-800/80 animate-pulse" />
            <div className="h-11 w-full rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 animate-pulse" />
          </div>
          <div className="space-y-2">
            <div className="h-3 w-24 rounded bg-slate-200/80 dark:bg-slate-800/80 animate-pulse" />
            <div className="h-11 w-full rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 animate-pulse" />
          </div>
        </div>

        {/* Field 3: Textarea */}
        <div className="space-y-2">
          <div className="h-3 w-32 rounded bg-slate-200/80 dark:bg-slate-800/80 animate-pulse" />
          <div className="h-28 w-full rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 animate-pulse" />
        </div>

        {/* Field 4: Skills Input */}
        <div className="space-y-2">
          <div className="h-3 w-28 rounded bg-slate-200/80 dark:bg-slate-800/80 animate-pulse" />
          <div className="h-11 w-full rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-slate-800 animate-pulse" />
        </div>

        {/* Buttons Skeleton */}
        <div className="flex gap-3 pt-4 border-t border-slate-100 dark:border-slate-800/80">
          <div className="h-10 w-36 rounded-xl bg-violet-600/20 dark:bg-violet-600/30 animate-pulse" />
          <div className="h-10 w-24 rounded-xl bg-slate-100 dark:bg-slate-800 animate-pulse" />
        </div>
      </div>
    </div>
  );
}
